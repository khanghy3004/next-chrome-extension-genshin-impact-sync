chrome.storage.sync.get(['account_uid'], function (items) {
  chrome.action.setBadgeBackgroundColor({ color: "blue" });
  chrome.action.setBadgeTextColor({ color: "white" });
  function updateCounter() {
    getAllCookies(async (cookies) => {
      if (cookies.length > 0 && typeof items.account_uid !== 'undefined') {
        fetch(`https://bbs-api-os.hoyolab.com/game_record/genshin/api/dailyNote?server=os_asia&role_id=${items.account_uid}`, {
          "headers": {
